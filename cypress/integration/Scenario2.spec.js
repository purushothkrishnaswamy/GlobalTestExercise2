describe('UnSuccessfull Checkout TestSuite', () => {
  ...
  //    // Test with empty Postal code while checkout
  it('cant checkout without all mandatory data', () => {
    cy.get('.shopping_cart_link').click()
    ...
    cy.fixture('users').then(function (data_checkout) {
      ...
      cy.log('Test without Postal code')
      cy.get('[data-test="firstName"]').type(this.data_checkout.FirstName1)
      cy.get('[data-test="lastName"]').type(this.data_checkout.LastName1)
